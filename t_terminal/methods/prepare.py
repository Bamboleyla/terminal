...
def prepare_date(broker, strategy) -> pd.DataFrame:
    ...
    tickers = strategy.get_config()["tickers"]
    data = pd.DataFrame()

    # 2. Process each ticker in the list
    for ticker in tickers:
        ticker_dir = os.path.join("t_terminal\data", ticker["tiker"])
        ...
        if not os.path.exists(ticker_dir):
            ...
            data = broker.get_candles(
                instrumentId=ticker["figi"], start_date=from_date, end_date=now
            )
        # 2.1 If ticker's data directory exists
            ...
            # 2.3. Set time range for new data load (last date in file)
            last_date = (
                pd.to_datetime(data["DATE"].iloc[-1]) - pd.Timedelta(hours=3)
            ).strftime("%Y-%m-%dT%H:%M:%SZ")
            # 2.4. Load new data into ticker's directory
            new_candles = broker.get_candles(
                instrument_id=ticker["figi"], start_date=last_date, end_date=now
            )

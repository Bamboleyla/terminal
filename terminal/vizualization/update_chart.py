    ...
    for indicator in ticker_config["indicators"]:
        if indicator["type"] == "super_trend":
            ...
            indicator_data = pd.read_csv(
                f'terminal/data/SBER/indicators/{indicator["id"]}.csv'
            ).iloc[-100:]
            indicator_data = pd.concat(
                [indicator_data.iloc[:-1], new_quotes]
            ).reset_index(drop=True)
            indicator_data = indicator_data.astype(
                {
                    col: "float64"
            ...
            # Calculate indicator
            updated_indicator_data = super_trend.calculate_indicator(
                indicator_data, indicator, 99
            )

            # Update data for display
            terminal_data[f'{indicator["show"][0]["column"]}'] = updated_indicator_data[
                "ST_UPPER"
            ]
            terminal_data[f'{indicator["show"][1]["column"]}'] = updated_indicator_data[
                "ST_LOWER"
            ]

...
def show_chart(ticker_config):
    ...
    max_date = plot_data.index.max()
    plot_data = plot_data[plot_data.index.date == max_date.date()]
    ...
    num_indicators = sum(
        len(indicator["show"]) for indicator in ticker_config["indicators"]
    )
    live_objects = fplt.live(
        1 + num_indicators
    )  # The first object is candles, the rest are indicators' lines

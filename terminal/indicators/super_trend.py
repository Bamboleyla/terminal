"""In this file there is a class of Super Trend indicator"""
...
class Super_Trend:
    ...
    def calculate_indicator(
        self, quotes: pd.DataFrame, indicator: dict, start_index: int = None
    ) -> pd.DataFrame:
        ...
        # Initialize ST_UPPER and ST_LOWER columns if they don't exist
        ...
        trend = (
            ...
            if start_index is None
                ...
                "lower"
                if not pd.isna(data.loc[start_index - 1, "ST_LOWER"])
                else "upper"
            )
        )
        ...
        for index, row in data.iterrows():
            ...
            upper = (
                round(row["UPPER_LINE"], 2)
                if pd.isnull(data.loc[index - 1, "ST_UPPER"])
                else round(min(data.loc[index - 1, "ST_UPPER"], row["UPPER_LINE"]), 2)
            )
            lower = (
                round(row["LOWER_LINE"], 2)
                if pd.isnull(data.loc[index - 1, "ST_LOWER"])
